...
class PredictResource(Resource):
    ...
    def get(self):
        ...
        model_file = os.path.join(current_app.config['MODEL_FOLDER'], self.model_name + '.model')
        label_file = os.path.join(current_app.config['MODEL_FOLDER'], self.model_name + '_lbls.pickle')
        config_file = current_app.config['MODEL_CONFIG_FILE']
        obj = Predict(image, self.model_name, model_file, label_file, config_file=config_file)
        res = obj.prediction()
        label = res['label']
