...
def model_requirements(argument, img_size):
    ...
    if safe_str_cmp(argument, "inceptionv3_pretrained"):
        ...
        if img_size[0] < inception_v3_min_dim or img_size[1] < inception_v3_min_dim:
            print(">ia> Input size for Inception v3 should be at least {}*{}; got {}".format(inception_v3_min_dim,
                                                                                             inception_v3_min_dim,
                                                                                             img_size))
            return "model_error"
    return 'OK'
    ...
    if not os.path.isdir('raw/' + dataset):
        if not os.path.isfile(np_dataset + '.npy'):
            if not downlaod_dataset_if_not_exists:
                ...
                3) Set download_ai2r_dataset in config.json to true. ".format('raw/' + dataset, data_name, dataset,
                                                                              dataset))
                return "input_error"
    return 'OK'
