...
def inceptionv3_pretrained(img_size, lb):
    """
    This function will take the pre-trained InceptionV3 model from Keras and modify
    the input size, number of classes, or both based on user input.
    """
    from models.inception import InceptionV3_pt
    print(">ia> Building pre-trained inceptionV3 model...")
...
def main():
    ...
    download_dataset_if_not_exists = config['dataset']['download_ai2r_dataset']
    if download_dataset_if_not_exists:
        use_raw_data = False
    ...
    r = check.input_requirements(dataset, np_dataset, download_dataset_if_not_exists)
    if not safe_str_cmp(r, "OK"):
        print(">ia> Exited with error: {}".format(r))
        return 0

    if download_dataset_if_not_exists:
        if not os.path.isfile(datasets_dir + '/aircrafts.npy'):
